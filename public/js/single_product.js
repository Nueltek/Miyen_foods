...
function generateProductDetails() {
    ...
    productName.textContent = `${productData[0].name}`;
    singleProductDetailsDiv.appendChild(productName);
  
    const productPrice = document.createElement('h2');
    productPrice.textContent = `$${productData[0].price}`;
    ...
    quantityDiv.innerHTML = `
      <div class="qty2">
        <span id="decrement-btn" data-id="${productData[0].id}">-</span>
        <strong id="s_price">${productData[0].qty}</strong>
        <span id="increment-btn" data-id="${productData[0].id}">+</span>
      </div>
    `;
