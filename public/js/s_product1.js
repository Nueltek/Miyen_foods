...
function generateProductDetails() {
    ...
    productName.textContent = `${productData[1].name}`;
    singleProductDetailsDiv.appendChild(productName);
  
    const productPrice = document.createElement('h2');
    productPrice.textContent = `$${productData[1].price}`;
    ...
    quantityDiv.innerHTML = `
      <div class="qty2">
        <span id="decrement-btn" data-id="${productData[1].id}">-</span>
        <strong id="s_price">${productData[1].qty}</strong>
        <span id="increment-btn" data-id="${productData[1].id}">+</span>
      </div>
    `;
