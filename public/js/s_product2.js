...
function generateProductDetails() {
    ...
    productName.textContent = `${productData[2].name}`;
    singleProductDetailsDiv.appendChild(productName);
  
    const productPrice = document.createElement('h2');
    productPrice.textContent = `$${productData[2].price}`;
    ...
    quantityDiv.innerHTML = `
      <div class="qty2">
        <span id="decrement-btn" data-id="${productData[2].id}">-</span>
        <strong id="s_price">${productData[2].qty}</strong>
        <span id="increment-btn" data-id="${productData[2].id}">+</span>
      </div>
    `;
